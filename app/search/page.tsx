...
import { listAnalystPrompts, listChatThreads, listFiles, listResearchItems } from "@/lib/repositories";

export default async function SearchPage() {
  const [prompts, libraryFiles, researchItems, chatThreads] = await Promise.all([
    listAnalystPrompts(),
    listFiles(),
    listResearchItems(),
    listChatThreads()
        ...
        <section className="rounded-[1.8rem] border border-border/80 bg-card/90 px-6 py-7 shadow-whisper">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h2 className="mt-4 font-serif text-4xl text-foreground">Find files, research, and threads</h2>
            </div>
            <RuntimeModeBadge />
          </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="relative">
              ...
              <Input className="pl-11" placeholder="Search files, research items, chats, or insights..." />
            </div>
            <div className="grid gap-6 md:grid-cols-3">
              ...
              <div className="rounded-[1.4rem] bg-accent/70 p-5">
                <p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Research</p>
                <div className="mt-4 flex flex-wrap gap-2">
